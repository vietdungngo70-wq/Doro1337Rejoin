...
BOT_TOKEN = "MTQ3MzIxNDA1ODIzMTYyNzc5Ng.GpM_D8.dWVNT0F4Nd0ILx5sj3mQNwp8-fvKNo1DdvopzU"
...
class RobloxManager:
    ...
    def get_roblox_packages():
        ...
        try:
            ...
            result = subprocess.run(
                f"pm list packages {package_prefix} | sed 's/package://'", shell=True, capture_output=True, text=True)
    ...
    def kill_roblox_processes():
        ...
        for package_name in packages:
            if any(package_name in proc for proc in running):
                os.system(
                    f"nohup /system/bin/am force-stop {package_name} > /dev/null 2>&1 &")
        time.sleep(2)

    @staticmethod
    def kill_roblox_process(package_name):
        try:
            subprocess.run(["/system/bin/am", "force-stop", package_name],
    ...
    def launch_roblox(package_name, server_link):
        try:
            ...
            with status_lock:
                globals()[
                    "package_statuses"][package_name]["Status"] = f"\033[1;36mOpening Roblox...\033[0m"

            subprocess.run(['am', 'start', '-a', 'android.intent.action.MAIN', '-n',
                           f'{package_name}/com.roblox.client.startup.ActivitySplash'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            time.sleep(10)

            with status_lock:
                globals()[
                    "package_statuses"][package_name]["Status"] = f"\033[1;36mJoining Server...\033[0m"

            subprocess.run(['am', 'start', '-a', 'android.intent.action.VIEW', '-n',
                           f'{package_name}/com.roblox.client.ActivityProtocolLaunch', '-d', server_link], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            time.sleep(15)
            with status_lock:
                globals()[
                    "package_statuses"][package_name]["Status"] = "\033[1;32mJoined Roblox\033[0m"
        except Exception as e:
            with status_lock:
                globals()[
                    "package_statuses"][package_name]["Status"] = f"\033[1;31mLaunch Error\033[0m"
...
class WebhookManager:
    ...
    def start_webhook_thread():
        ...
        if webhook_interval > 0 and webhook_url:
            stop_webhook_thread = False
            webhook_thread = threading.Thread(
                target=WebhookManager.send_webhook_loop, daemon=True)
            webhook_thread.start()

    @staticmethod
    def send_webhook_loop():
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            try:
                console.print("[green]🚀 Khởi động Động cơ Doro OS Ultimate...[/green]")
                p_id = settings.get("place_id", "2753915549")
                
                # 1. Lấy mục tiêu sếp đã ghim ở Lệnh 7
                target_pkg = settings.get("package_prefix", "com.roblox.client")
                
                # Kiểm tra xem máy sếp đã cài app đó chưa
                if not os.path.exists(f"/data/data/{target_pkg}"):
                    console.print(f"[bold red]❌ App '{target_pkg}' chưa được cài đặt hoặc chưa cấp quyền Root![/bold red]")
                    time.sleep(3)
                    continue
                    
                console.print(f"[bold cyan]🔥 Đã khóa mục tiêu: {target_pkg}[/bold cyan]")
                
                # --- KÍCH HOẠT SỨC MẠNH DORO PRO MAX ---
                console.print("[dim]⚡ Đang ép xung Đồ họa (Unlock 120 FPS & Potato Mode)...[/dim]")
                DoroProMax.unlock_fps_and_potato_mode(target_pkg)
                
                # 2. MA THUẬT QUÉT NÓNG: Đọc thẳng file hệ thống
                console.print("[dim]🔍 Tiến hành moi ID (Quét Nóng)...[/dim]")
                instant_id = Runner.quick_scan_for_pkg(target_pkg)
                
                server_links = [(target_pkg, f"roblox://placeID={p_id}")]
                RobloxManager.kill_roblox_processes()
                
                # 3. Nảy số lên Dashboard ngay lập tức & CHUẨN BỊ HANDSHAKE
                if instant_id:
                    globals()["_user_"][target_pkg] = instant_id
                    username = FileManager.get_username(instant_id)
                    
                    globals()["package_statuses"] = {
                        target_pkg: {
                            "Username": username,
                            "Status": "\033[1;33m⏳ Đang bơm game và chờ Executor...\033[0m"
                        }
                    }
                    console.print(f"[green]✅ Bắt trúng tim đen: {username} ({instant_id})[/green]")
                    FileManager.save_accounts([(target_pkg, instant_id)])
                    
                    # ---> ĐÂY RỒI: DỌN RÁC & BƠM MỒI NHỬ VÀO AUTOEXEC <---
                    console.print("[dim]💉 Đang chuẩn bị File mồi (Handshake) cho pháp sư...[/dim]")
                    DoroProMax.prepare_payload(instant_id)
                    
                else:
                    globals()["_user_"][target_pkg] = "Unknown"
                    globals()["package_statuses"] = {
                        target_pkg: {
                            "Username": "Chưa rõ 🔍",
                            "Status": "\033[1;33mKhởi động mù (Blind Launch)...\033[0m"
                        }
                    }
                    console.print("[yellow]⚠️ Chưa thấy nick đăng nhập. Tool sẽ tự rình...[/yellow]")
                ...
                # 5. Kích hoạt "Ống nhòm" soi Executor chạy ngầm (Chỉ chạy nếu có ID)
                if instant_id:
                    threading.Thread(target=DoroProMax.watch_and_rejoin, args=(target_pkg, f"roblox://placeID={p_id}", instant_id), daemon=True).start()

                # 6. Duy trì hệ thống & Nuôi chó canh cửa
            ...
            try:
                print("\033[1;36m-- WEBHOOK SETUP --\033[0m")

                wb_url = input("URL: ").strip()
                wb_name = input("Name: ").strip()
                wb_int = int(input("Interval (Min): ").strip())

                settings = FileManager.load_advanced_settings()
                settings["webhook_url"] = wb_url
                settings["device_name"] = wb_name
                settings["webhook_interval"] = wb_int
                FileManager.save_advanced_settings(settings)

                webhook_url = wb_url
                device_name = wb_name
                webhook_interval = wb_int
                ...
                console.print("[green]✅ Saved & Webhook Restarted![/green]")

            except Exception as e:
                console.print(f"[red]❌ Lỗi Webhook: {e}[/red]")
            ...
            try:
                ...
                os.makedirs("Shouko.dev", exist_ok=True)
                db_path = "Shouko.dev/Cookies.db"
                app_path = "Shouko.dev/appStorage.json"
