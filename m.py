    ...
    while True:
        ...
        if choice == "1":
            try:
                ...
                # 1. Tự động tìm Package (Hỗ trợ Clone có đuôi lạ)
                prefix = settings.get("package_prefix", "com.roblox.client")
                target_pkg = subprocess.getoutput(f"pm list packages | grep '{prefix}' | sed 's/package://' | head -n 1").strip()
                
                if not target_pkg or not os.path.exists(f"/data/data/{target_pkg}"):
                    console.print(f"[bold red]❌ App '{prefix}' chưa cài hoặc chưa có Root![/bold red]")
                    time.sleep(3)
                    continue
                ...
                # 2. Moi ID trực tiếp bằng FileManager của tool gốc (Fix lỗi Runner)
                console.print("[dim]🔍 Tiến hành moi ID (Quét Nóng)...[/dim]")
                file_path = f'/data/data/{target_pkg}/files/appData/LocalStorage/appStorage.json'
                instant_id = FileManager.find_userid_from_file(file_path)
                
                server_links = [(target_pkg, f"roblox://placeID={p_id}")]
                RobloxManager.kill_roblox_processes()
                
                # 3. Nảy số & Chuẩn bị File Lua
                ...
                # 5. Kích hoạt Ống nhòm (Rình file .main báo thành công)
                if instant_id != "Unknown":
